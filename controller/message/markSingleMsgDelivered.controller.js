...
const markSingleMsgDelivered=async (req,res)=>{
    ...
    try{
        const msg=await messageModel.updateOne({_id:msgId,chatId,senderId:{$ne:user._id},isRead:"unread"},{$set:{isRead:"delivered"}});
        if(msg.modifiedCount==0)    return res.status(401).json({msg:"invalid msg..."});
        return res.json({msg:"mark delivered"});
    }catch(e){
        return res.status(500).json({msg:"server Error"});
    }
}

module.exports=markSingleMsgDelivered;